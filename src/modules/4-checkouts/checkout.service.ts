...
export class CheckoutService {
  ...
  async checkoutReview({ carId, userId, shopOrders }: CheckoutReviewDto) {
    ...
    for (let i = 0; i < shopOrders.length; i++) {
      ...
      const checkoutProductsServer = await this.cartService.checkoutProductsByServer(items);

      if (!checkoutProductsServer.length) throw new BadRequestException("Order wrong!!! ");
  ...
  async orderByUser({ shopOrders, carId, userId, userAddress = {}, userPayment = {} }: any) {
