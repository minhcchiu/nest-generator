...
export class ProvinceController {
  constructor(private readonly provinceService: ProvinceService, private readonly districtService: DistrictService, private wardService: WardService) { }
  ...
  async findAll(
    ...
    const [district, province, ward] = await Promise.all([this.provinceService.deleteMany({}), this.districtService.deleteMany({}), this.wardService.deleteMany()])
    ...
    if (idFileExist) {
      ...
      try {
        ...
        const resultPromise = data.map((tinh: any,) => {
          ...
          const tinhPromise = this.provinceService.create({ name: tinh.name, type: tinh.type })
          ...
          return tinhPromise.then(rsTinh => {
            ...
            return tinh.level2s.map((huyen: any) => {
              ...
              const huyenPromise = this.districtService.create({ name: huyen.name, type: huyen.type, idProvince: idTinh })

              huyenPromise.then(rsHuyen => {

                return huyen.level3s.map((xa: any) => {

                  const xaPromise = this.wardService.create({ name: xa.name, type: xa.type, idProvince: idTinh, idDistrict: rsHuyen._id })
